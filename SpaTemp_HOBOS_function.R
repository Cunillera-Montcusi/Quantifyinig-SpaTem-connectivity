...
ifelse(is.character(print.directory)==T,
   "Hope that the plot.directory is the directory where you want to save the photos with a last / ",
...
spa_connections <-seq(1,ncol(HOBOS_dataset[[river]]),1)
time_step_1 <- rep(1,ncol(HOBOS_dataset[[river]])-1)
for (site_step in 1:c(ncol(HOBOS_dataset[[river]])-2)) {
...
Simple_river_network_maps[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
...
spa_connections <-seq(1,length(colnames(HOBOS_dataset[[river]])),1)+((days-1)*numn_nodes)
...
if(temp_change==0){# Temporal change is constant 
  if(time_step_1[site_step]==1){# This temporal change implies going from 1 to 1 (so a real stable connected link)
    ...
    # Tot_Num indicator.  
    ST_matrix[temp_connections[site_step],
              temp_connections[site_step]+numn_nodes] <- value_T_LINK #1
...
if(temp_change==1){
  ...
  All_river_paths[upper.tri(All_river_paths)] <- value_NO_T_link
   for (every_path in site_step:c(length(time_step_1)-1)){
    ...
    All_river_paths[every_path,site] <- value_T_LINK #1
  }
  ST_matrix[temp_connections[site_step],
            c(temp_connections[1]+numn_nodes):c(temp_connections[numn_nodes]+numn_nodes)] <- All_river_paths[site_step,]
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  ST_matrix<- ST_matrix_rivers[[river]][1:c(numn_nodes*length(HOBOS_dataset[[river]]$Day)),
                                        1:c(numn_nodes*length(HOBOS_dataset[[river]]$Day))]
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  ST_Oclo_plot[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
                                  aes(x = x, y = y, xend = xend, yend = yend))+
    geom_edges(aes(color =edges_CC_values, size=edges_CC_values),arrow=arrow(angle = 20),curvature = 0.15) +
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  ST_Allclo_plot[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
                                  aes(x = x, y = y, xend = xend, yend = yend))+
    geom_edges(aes(color =edges_CC_values, size=edges_CC_values),arrow=arrow(angle = 20),curvature = 0.15) +
    geom_nodes(aes(size=Sd_values),fill="grey50", color="black" ,shape=21, alpha=0.2)+
    geom_nodes(aes(fill=CC_values, size=CC_values), color="black" ,shape=21)+
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  ST_betw_plot[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
                                  aes(x = x, y = y, xend = xend, yend = yend))+
...
Main_output <- list(Dir_NonW_Main_matrix=  NonW_ST_matrix_rivers,
                    Dir_NonW_ST_matrix=    NonW_ST_matrix_out_out,
                    Dir_NonW_ST_matrix_plo=NonW_ST_matrix_plots,
                    Dir_NonW_ST_con=       NonW_ST_connectivity_value,
                    Dir_NonW_ST_con_plo=   NonW_ST_connectivity_plot,
                    Dir_NonW_ST_Ocl=       NonW_ST_directed_Ocloseness_rivers,
                    Dir_NonW_ST_Ocl_plo=   NonW_ST_Oclo_plot,
                    Dir_NonW_ST_Acl=       NonW_ST_directed_Allcloseness_rivers,
                    Dir_NonW_ST_Acl_plo=   NonW_ST_Allclo_plot,
                    Dir_NonW_ST_Bet=       NonW_ST_directed_betweennes_rivers,
                    Dir_NonW_ST_Bet_plo=   NonW_ST_betw_plot)
return(Main_output)
####_______________________________________________________________________
}
...
Main_output <- list(Dir_WEIG_Main_matrix=  WEIG_ST_matrix_rivers,
                    Dir_WEIG_ST_matrix=    WEIG_ST_matrix_out_out,
                    Dir_WEIG_ST_matrix_plo=WEIG_ST_matrix_plots,
                    Dir_WEIG_ST_con=       WEIG_ST_connectivity_value,
                    Dir_WEIG_ST_con_plo=   WEIG_ST_connectivity_plot,
                    Dir_WEIG_ST_Ocl=       WEIG_ST_directed_Ocloseness_rivers,
                    Dir_WEIG_ST_Ocl_plo=   WEIG_ST_Oclo_plot,
                    Dir_WEIG_ST_Acl=       WEIG_ST_directed_Allcloseness_rivers,
                    Dir_WEIG_ST_Acl_plo=   WEIG_ST_Allclo_plot,
                    Dir_WEIG_ST_Bet=       WEIG_ST_directed_betweennes_rivers,
                    Dir_WEIG_ST_Bet_plo=   WEIG_ST_betw_plot)
return(Main_output)
####_______________________________________________________________________
  }
}
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  # We extract the main matrix that corresponds to the river
  Un_ST_matrix<- Un_ST_matrix_rivers[[river]][1:c(numn_nodes*length(HOBOS_dataset[[river]]$Day)),
                                              1:c(numn_nodes*length(HOBOS_dataset[[river]]$Day))]
  ...
  out <- foreach(nodes=1:numn_nodes)%dopar%{
  #for (nodes in 1:numn_nodes) {
    spt_conn[nodes] <- sum(apply(Un_ST_matrix,1,sum)[seq(nodes,numn_nodes*length(HOBOS_dataset[[river]]$Day)-numn_nodes,numn_nodes)])/2
  }
  # We divide by the number of days so we obtain the "per day" values. max (((numn_nodes*2-1)*512)/position of the node)/513
  spt_conn <- unlist(out)
  spt_conn<- spt_conn/c(length(HOBOS_dataset[[river]]$Day))
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  Un_ST_Oclo_plot[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
                                  aes(x = x, y = y, xend = xend, yend = yend))+
    geom_edges(aes(color =edges_CC_values, size=edges_CC_values/2),arrow=arrow(angle = 20,ends = "both" ),curvature = 0.15) +
    geom_nodes(aes(size=Sd_values),fill="grey50", color="black" ,shape=21, alpha=0.2)+
    geom_nodes(aes(fill=CC_values, size=CC_values), color="black" ,shape=21)+
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  Un_ST_Allclo_plot[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
                                     aes(x = x, y = y, xend = xend, yend = yend))+
    geom_edges(aes(color =edges_CC_values, size=edges_CC_values/2),arrow=arrow(angle = 20,ends = "both" ),curvature = 0.15) +
    geom_nodes(aes(size=Sd_values),fill="grey50", color="black" ,shape=21, alpha=0.2)+
    geom_nodes(aes(fill=CC_values, size=CC_values), color="black" ,shape=21)+
...
grid.arrange(Un_ST_Allclo_plot[[1]],Un_ST_Allclo_plot[[2]],Un_ST_Allclo_plot[[3]],
             ...
             Un_ST_Allclo_plot[[7]], top="UnD NonW ST All closennes")
dev.off()}
...
for (river in 1:length(HOBOS_dataset)) {
  ...
  Un_ST_betw_plot[[river]] <- ggplot(n, layout=as.matrix(Sites_coordinates[[river]][,4:3]),
                                  aes(x = x, y = y, xend = xend, yend = yend))+
...
Main_output <- list(UnD_NonW_Main_matrix=  Un_NonW_ST_matrix_rivers,
                    UnD_NonW_ST_matrix=    Un_NonW_ST_matrix_out_out,
                    UnD_NonW_ST_matrix_plo=Un_NonW_ST_matrix_plots,
                    UnD_NonW_ST_con=       Un_NonW_ST_connectivity_value,
                    UnD_NonW_ST_con_plo=   Un_NonW_ST_connectivity_plot,
                    UnD_NonW_ST_Ocl=       Un_NonW_ST_directed_Ocloseness_rivers,
                    UnD_NonW_ST_Ocl_plo=   Un_NonW_ST_Oclo_plot,
                    UnD_NonW_ST_Acl=       Un_NonW_ST_directed_Allcloseness_rivers,
                    UnD_NonW_ST_Acl_plo=   Un_NonW_ST_Allclo_plot,
                    UnD_NonW_ST_Bet=       Un_NonW_ST_directed_betweennes_rivers,
                    UnD_NonW_ST_Bet_plo=   Un_NonW_ST_betw_plot)
return(Main_output)
####_______________________________________________________________________
}
if(weighting==TRUE){ # Weighting
...
Main_output <- list(UnD_WEIG_Main_matrix=  Un_WEIG_ST_matrix_rivers,
                    UnD_WEIG_ST_matrix=    Un_WEIG_ST_matrix_out_out,
                    UnD_WEIG_ST_matrix_plo=Un_WEIG_ST_matrix_plots,
                    UnD_WEIG_ST_con=       Un_WEIG_ST_connectivity_value,
                    UnD_WEIG_ST_con_plo=   Un_WEIG_ST_connectivity_plot,
                    UnD_WEIG_ST_Ocl=       Un_WEIG_ST_directed_Ocloseness_rivers,
                    UnD_WEIG_ST_Ocl_plo=   Un_WEIG_ST_Oclo_plot,
                    UnD_WEIG_ST_Acl=       Un_WEIG_ST_directed_Allcloseness_rivers,
                    UnD_WEIG_ST_Acl_plo=   Un_WEIG_ST_Allclo_plot,
                    UnD_WEIG_ST_Bet=       Un_WEIG_ST_directed_betweennes_rivers,
                    UnD_WEIG_ST_Bet_plo=   Un_WEIG_ST_betw_plot)
return(Main_output)
#_______________________________________________________________________ ####
    }# Weighting
  }# Direction
